...
static void launch_app(DFlist *df_list, char *response) {
    struct Dentry **result = NULL;
    for (int ctr = 0; ctr < df_list->dentr_ctr; ++ctr){ // bsearch() failes in some cases, replaced with linear
        if (strcasecmp(df_list->dentries[ctr]->app_name, response) == 0 && !df_list->dentries[ctr]->hid) {
            result = &df_list->dentries[ctr];
            break;
        };
    };

    if (!result) { printf("[dmenu_desktop] dmenu response \"%s\" not found in app list.\n", response); return; };

    if (fork() == 0) {
        if ((*result)->term) { execlp(TERM, TERM, "-e", "sh", "-c", (*result)->exec_path, NULL);
        } else { execlp("sh", "sh", "-c", (*result)->exec_path, NULL); };
        printf("[dmenu_desktop] Failed to launch application. Exec path: %s\n", (*result)->exec_path);
    }
}

static void exec_dmenu_cmd(char *dmenu_comm, char ***sel_apps, unsigned long *sel_apps_num){
    // Open dmenu, read and execute selected app
    FILE *pipe = popen(dmenu_comm, "r");
    if (!pipe) { perror("[dmenu_desktop] Failed to open dmenu pipe"); return; };
...
static char* constr_dmenu_cmd (DFlist *df_list){
    // counting length of all visible app names
    unsigned long names_str_len = 0;
    for (int ctr = 0; ctr < df_list->dentr_ctr; ++ctr){
        if (!df_list->dentries[ctr]->hid) { names_str_len += strlen(df_list->dentries[ctr]->app_name) + 1; };
    };
    // combine names into one string using new line as separator
    char *names_str = calloc(names_str_len, sizeof(char));
    if (!names_str){ perror("[dmenu_desktop] ERROR: calloc: "); return NULL; };
    ...
    char *command;
    asprintf(&command, "echo \"%s\" | dmenu %s -i -g %d -l %d -p \"%s\"", names_str, POSITION, COLNUM, ROWNUM, DM_PT);
    free(names_str);

    return command;
}

static void free_df_list(DFlist *df_list){
    for(int ctr = 0; ctr < df_list->dentr_ctr; ++ctr) {
...
static void process_pair (struct Dentry *a, struct Dentry *b){
...
static int parse_dfile (struct Dentry *dentry){
    FILE *file = fopen(dentry->df_path, "r");
    if (!file){ fprintf(stderr, "[dmenu_desktop] ERROR: %s: ", dentry->df_path); perror(""); return 1; };
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (!fl_an && !strncmp(line, "Name=", 5)) { dentry->app_name = strdup(line + 5); ++fl_an;
            ...
            ++fl_ep;
        } else if (!dentry->term && !strcmp(line, "Terminal=true")) { dentry->term = 1;
        } else if (!dentry->hid && (!strcmp(line, "Hidden=true") || !strcmp(line, "NoDisplay=true"))) { dentry->hid = 1; };
        // In case of hidden entry we care only about app name
        if (fl_an && dentry->hid) { break; };
    };
    // Hide entries without a name
    if (!fl_an) { dentry->app_name = strdup("N/A"); dentry->hid = 1; };
    // or execution path
    if (!fl_ep) { dentry->exec_path = strdup("N/A"); dentry->hid = 1; };
...
static int parse_paths(char **paths, size_t num_paths, DFlist *df_list) {
    for (size_t ctr = 0; ctr < num_paths; ++ctr) {
        // get dirent list of .desktop files
        struct dirent **de_list;
        int de_ctr = scandir(paths[ctr], &de_list, filter, NULL);
        if (de_ctr < 0) { fprintf(stderr, "[dmenu_desktop] ERROR: %s: ", paths[ctr]); perror(""); return 1; };
        if (de_ctr == 0) { continue; };
        // resize dentries array to fit new entries
        struct Dentry **tptr = NULL;
        tptr = realloc(df_list->dentries, (df_list->dentr_ctr + de_ctr) * sizeof(struct Dentry*));
        if (!tptr) { perror("[dmenu_desktop] ERROR: realloc: "); return 1; };
        df_list->dentries = tptr;
        // fill .desktop entries data
        for (int ctr2 = 0; ctr2 < de_ctr; ++ctr2) {
            df_list->dentries[df_list->dentr_ctr] = calloc(1, sizeof(struct Dentry));
            if (!df_list->dentries[df_list->dentr_ctr]) { perror("[dmenu_desktop] ERROR: calloc: "); return 1; };
            // fill full path to .desktop entry
            asprintf(&df_list->dentries[df_list->dentr_ctr]->df_path, "%s/%s", paths[ctr], de_list[ctr2]->d_name);
...
int main (int argc, char **argv) {
    ...
    DFlist df_list = {0};
    if (parse_paths(&argv[1], argc-1, &df_list)){ return 0; };
    ...
    for (int ctr = 0; ctr < df_list.dentr_ctr-1; ++ctr){ process_pair(df_list.dentries[ctr], df_list.dentries[ctr+1]); };
    // form dmenu command
    char *dmenu_comm = constr_dmenu_cmd(&df_list);
    if (!dmenu_comm) { free_df_list(&df_list); return 0; };
    // execute dmenu command
    char **sel_apps = NULL;
    unsigned long sel_apps_num = 0;
    exec_dmenu_cmd(dmenu_comm, &sel_apps, &sel_apps_num);
    free(dmenu_comm);
    // launch app(s)
    if (sel_apps_num > 0){
        for (int i = 0; i < sel_apps_num; ++i){ launch_app(&df_list, sel_apps[i]); free(sel_apps[i]); };
        free(sel_apps);
    };
