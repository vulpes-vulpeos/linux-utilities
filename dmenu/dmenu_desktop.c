...
#define TERM   "kitty"
#define DM_PT  "Search:" // prompt to the left of the input field
...
void launch_app(DFlist *dflist, char *response) {
    ...
    if (!result || (*result)->hid) { printf("[dmenu_desktop] dmenu response \"%s\" not found in app list.\n", response); return; };
...
void exec_comm(char *dmenu_comm, char ***sel_apps, unsigned long *sel_apps_num){
...
char* constr_comm (DFlist *df_list){
    ...
    char *command;
    asprintf(&command, "echo \"%s\" | dmenu -b -i -p \"%s\"", names_str, DM_PT);
    free(names_str);
...
int parse_paths(char **paths, size_t num_paths, DFlist *df_list) {
    for (size_t i = 0; i < num_paths; ++i) {
        ...
        if (de_ctr < 0) { fprintf(stderr, "[dmenu_desktop] ERROR: %s: ", paths[i]); perror(""); return 1; };
